...
class TDMPC():
	...
	def ICM(self, obs, next_obs, action):
		...
		with torch.no_grad():
			# extract latent representations using online encoder
			target_z = self.model.h(self.aug(obs))
			next_target_z = self.model.h(self.aug(next_obs))

			# compute intrinsic reward as the error in predicting next latent state
